...
- **Novo `src/game/tilemap.ts`** — `TileType` ('grama' | 'agua'), uma matriz 2D (`TileGrid`) descrevendo o tile de cada célula, e funções de conversão mundo↔célula. O caminho não faz parte da grade (ver Fase 2) — é uma camada de sprites carimbados independente, em ângulo livre.
...
- **Nada de "pulo" a cada 3min:** os 5 nomes (amanhecer/dia/tarde/entardecer/noite) são só rótulos de âncora, não estados discretos que a luz salta entre si. Tudo que deriva do horário — cor/alpha do overlay, `sunAngle`, sombra dos props — é uma função contínua de `survivalTimeMs` (`progress = clamp(survivalTimeMs / cycleDurationMs, 0, 1)`, interpolação linear entre a âncora atual e a próxima). Recalculado a cada tick do game loop, não por estágio.
- **Direção de luz:** `sunAngle` (0–360°) é essa mesma função contínua de `progress` — baixo/lateral ao amanhecer/entardecer, alto/quase vertical ao meio-dia, irrelevante à noite (sombra some gradualmente, não some de uma vez).
- **Sombra dos props:** cada prop estático ganha uma sombra pareada — uma elipse escura semi-transparente (`Graphics`), cujo comprimento/rotação são recalculados a partir de `sunAngle` a cada ~500ms (throttle de redesenho, não de progresso — a luz em si já mudou continuamente nesse meio tempo, só o redesenho da sombra é que não precisa ser todo frame).
- **Sombreado direcional nos tiles:** além do overlay global e da sombra dos props, cada tile visível ganha um gradiente direcional sutil desenhado em código (não faz parte da arte do tile, que continua neutra/meio-dia como definido na Fase 1) — mais claro do lado voltado pro sol, mais escuro do lado oposto, seguindo `sunAngle`. Implementado como uma camada `Graphics` (ou `Mesh` com gradiente) por cima de cada tile, recalculada no mesmo throttle de ~500ms das sombras. É o que dá a sensação de luz mudando de direção no cenário como um todo, não só nos props isolados.
- **Onde vive o estado:** `sunAngle`/progresso do horário ficam em `GamePage.vue`/`scene.ts` (não na store Pinia) — mesmo padrão já usado pra `effectsState`/`combatFx`, estado de motor de jogo fica fora do Pinia.
